...
import { getBooks, fetchUserById, saveBook } from "../api/api";
...
const Search = () => {
    ...
    const handleAddToWishlist = async (bookId: number | undefined) => {
        ...
        try {
            await saveBook({ user_id: currentUser.user_id, book_id: bookId });
            alert("Book added to your wishlist! 📚");
        } catch (error) {
            const axiosError = error as AxiosError;
            console.error("Error adding to wishlist:", axiosError.response?.data || error);
                            ...
                            <div className="flex gap-6 text-brown-0">
                                ...
                                <div>
                                    ...
                                    <div className="flex gap-6 text-md mt-5">
                                        <button className="btn-primary-br w-36 py-2 px-4 hover:glow-brown">Borrow</button>
                                        <button className="btn-primary-br w-36 py-2 hover:glow-brown">Exchange</button>
                                        <button className="btn-primary-br w-36 py-2 hover:glow-brown"
                                                onClick={() => handleAddToWishlist(activeBook.book_id)}>+ Wishlist</button>
                                    </div>
                                </div>
                            </div>
                        </div>
